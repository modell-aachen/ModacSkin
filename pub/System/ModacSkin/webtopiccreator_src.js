foswiki.webTopicCreator = {
     ...
     */
		...
		/**
		 * Mit dem Ajax Request können Abfragen getätigt werden
		 */
	_ajaxRequest: function(web, topic) {	
		$.ajax( 				
			{
				url: 'http://127.0.0.1/svnfoswiki2/bin/view/Sandbox/AjaxHelper?section=newtopic;contenttype=text/plain;limit=all;skin=text;input=' + topic + ';baseweb=' + web + ';format=$web.$topic,$topic;',
	...
	/**
	 * Überprüfung, ob das neue Topic bereit zum Erstellen ist.
	 */
    _canSubmit: function(inForm, inShouldConvertInput, ajaxExec) {
		var webname, template, parenttopic, topic, searchstring;
		
		//Alex: angepasst an Q.Wiki Dokumentenstruktur
		//Alex: 
		...
		if (inputForTopicName.length == 0) {
			...
			{
			template = inputForTemplate;
			
			/* Template überprüfen */
			//alert(template);
			switch (inputForTemplate) {
			  	//Beispiel:
				case "Main.GroupTemplate":
				  //ergebnis = inputForTopicName;
					
			      searchstring = "*Template";
				  ergebnis = inputForTopicName.match(/^(([\S][\S]-)(\d\d)(-)(\d\d))$/g);
				  $("#webTopicCreatorFeedback").html("");
				  foswiki.HTML.setHtmlOfElementWithId("webTopicCreatorFeedback", "Zwingendes Eingabeformat: $$-##-##. Beispiel: US-01-01");
				  break;
			  default:
				ergebnis = inputForTopicName;
			  
			  	searchstring = "Web*";
			  		//ergebnis = inputForTopicName.match(/(US-)(\d\d)/g);
			  	$("#webTopicCreatorFeedback").html("");
			  	foswiki.HTML.setHtmlOfElementWithId("webTopicCreatorFeedback", "Bitte achten Sie auf die Q.Wiki Namenskonventionen");
...
(function($) {
    $(document).ready(
        function() {
            ...
            .change(
                function(e) {
                	foswiki.webTopicCreator.ajaxRequest(inForm.webname.value, "Web*");
                    foswiki.webTopicCreator._canSubmit(this.form,false);
                })
            .blur(
                function(e) {
